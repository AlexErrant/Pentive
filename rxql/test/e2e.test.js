...
import { GRAPHQL_PORT } from "../shared"
...
fixture`Example page`.page`http://localhost:8888/?storage=${storage}`
...
test("insert/remove a hero", async (t) => {
  ...
  await t
    .expect(heroNameInput.value)
    .eql("", "input is empty")
    .typeText(heroNameInput, "BobKelso")
    .expect(heroNameInput.value)
    .contains("Kelso", "input contains name")

  // input color
  const heroColorInput = Selector("#insert-box input[name=color]")
  await t
    .expect(heroColorInput.value)
    .eql("", "input is empty")
    .typeText(heroColorInput, "black")
    .expect(heroColorInput.value)
    .contains("black", "input contains color")
  ...
  const heroElements = Selector("#heroes-list .hero-item")
  await t
    .expect(heroElements.textContent)
    .contains("Kelso", "list-item contains name")
...
test.page(
  "http://localhost:" +
    GRAPHQL_PORT +
    "/static/multitab.html?frames=2&storage=" +
    storage
)("replication: insert/delete hero and check other tab", async (t) => {
  ...
  await t
    .expect(heroListElement.textContent)
    .contains("Irwin", "list-item contains name")
