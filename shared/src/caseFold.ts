// Note that "Changes_When_NFKC_Casefolded" exists, but I'm not using it since casefolding in this file should be independent
...
export function caseFold(s: string) {
	return s.replaceAll(changesWhenCasefolded, (foldableChar) => {
		const folded = cft.get(foldableChar)
		if (folded == null) {
			console.warn(
				'Code point not found in the case folding map:',
				foldableChar.codePointAt(0),
			)
			return foldableChar
		}
		return folded
